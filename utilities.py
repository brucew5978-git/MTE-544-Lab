...
def euler_from_quaternion(quat):
    ...
    """
    # Yaw (z-axis rotation)
    sin_cosp = 2 * (quat.w * quat.z + quat.x * quat.y)
    cos_cosp = 1 - 2 * (quat.y * quat.y + quat.z * quat.z)
...
def calculate_linear_error(current_pose, goal_pose):
    ...
    # print("Linear Error: {}".format(error_linear))
    return error_linear
...
def calculate_angular_error(current_pose, goal_pose):
    ...
    target_angle = atan2(goal_pose[1] - current_pose[1], goal_pose[0] - current_pose[0]) 
    error_angular = target_angle - current_pose[2]

    # Remember to handle the cases where the angular error might exceed the range [-π, π]
    if error_angular > 3.14159 or error_angular < -3.14159: 
        error_angular = error_angular % 3.14159
